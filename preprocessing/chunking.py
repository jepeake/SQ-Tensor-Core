...
def save_chunks(
    ...
    for chunk, metadata in tqdm(all_chunks, desc="Saving Chunks"):
        ...
        if save_text:
            text_file = output_dir / f"chunk_{metadata.chunk_index:04d}.txt"
            with open(text_file, 'w') as f:
                f.write(f"Chunk {metadata.chunk_index} from {metadata.original_name}\n")
                f.write(f"Original shape: {metadata.original_shape}\n")
                f.write(f"Chunk shape: {metadata.chunk_shape}\n")
                f.write(f"Position: rows {metadata.row_start}-{metadata.row_end}, cols {metadata.col_start}-{metadata.col_end}\n")
                f.write("=" * 80 + "\n")
    ...
    with open(output_dir / "chunks_summary.txt", "w") as f:
        ...
        for matrix_name, chunks in by_matrix.items():
            ...
            if save_text:
                f.write(f"  Text files: chunk_{chunks[0].chunk_index:04d}.txt to chunk_{chunks[-1].chunk_index:04d}.txt\n")
    
    if save_text:
        with open(output_dir / "chunks_preview.txt", "w") as f:
            f.write("Matrix Chunks Preview\n")
            f.write("====================\n\n")
            ...
            for chunk, metadata in all_chunks:
                if metadata.original_name not in by_matrix:
                    by_matrix[metadata.original_name] = []
                by_matrix[metadata.original_name].append((chunk, metadata))
            
            for matrix_name, matrix_chunks in by_matrix.items():
                f.write(f"\n{'='*60}\n")
                f.write(f"Matrix: {matrix_name}\n")
                f.write(f"Original Shape: {matrix_chunks[0][1].original_shape}\n")
                f.write(f"Total Chunks: {len(matrix_chunks)}\n")
                f.write(f"{'='*60}\n\n")
                
                # Show First Few Chunks as Preview
                preview_count = min(3, len(matrix_chunks))
                for i in range(preview_count):
                    chunk, metadata = matrix_chunks[i]
                    f.write(f"Chunk {metadata.chunk_index} (first 5x5 values):\n")
                    f.write(f"Position: rows {metadata.row_start}-{metadata.row_end}, cols {metadata.col_start}-{metadata.col_end}\n")
                    
                    # Show First 5x5 Submatrix of Chunk
                    preview_rows = min(5, chunk.shape[0])
                    preview_cols = min(5, chunk.shape[1])
                    preview_chunk = chunk[:preview_rows, :preview_cols]
                    
                    for row in preview_chunk:
                        f.write(" ".join(f"{val:8.4f}" for val in row))
                        f.write("\n")
                    
                    if chunk.shape[0] > 5 or chunk.shape[1] > 5:
                        f.write(f"... (showing {preview_rows}x{preview_cols} of {chunk.shape[0]}x{chunk.shape[1]})\n")
                    f.write("\n")
                
                if len(matrix_chunks) > preview_count:
                    f.write(f"... and {len(matrix_chunks) - preview_count} more chunks\n\n")
...
def main():
    ...
    parser.add_argument("--save-text", action="store_true",
                       help="Save each chunk as a text file for easy checking")
    ...
    for matrix_name, matrix, info in tqdm(matrices, desc="Processing Matrices"):
        ...
        chunks = chunk_matrix(matrix, matrix_name, args.chunk_size, global_chunk_index)
        all_chunks.extend(chunks)
        global_chunk_index += len(chunks)
        print(f"  {matrix_name}: {matrix.shape} → {len(chunks)} chunks (indices {global_chunk_index - len(chunks)} to {global_chunk_index - 1})")
    
    print(f"Chunks Generated: {len(all_chunks)}")
    
    print(f"Saving Chunks to: {output_path}")
    save_chunks(all_chunks, output_path, args.save_text)
    
    if args.save_text:
        print(f"Text files saved for easy checking:")
        print(f"  Individual chunks: chunk_XXXX.txt")
        print(f"  Preview file: chunks_preview.txt")
